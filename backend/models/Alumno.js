...
class Alumno {
  ...
  static async create(alumnoData) {
    try {
      ...
      const [result] = await connection.execute(
        'INSERT INTO alumnos (nombre, apellidos, edad, telefono, clase, codigo, activo) VALUES (?, ?, ?, ?, ?, ?, 1)',
        [nombre, '', edad, telefono, clase, codigo || null]
      );
  ...
  static async generateUniqueCode(clase) {
    try {
      ...
      const claseKey = (clase || '').toString().toLowerCase();
      const prefijo = prefijos[claseKey] || 'GEN';
      let numero = 1;
      let codigo;
