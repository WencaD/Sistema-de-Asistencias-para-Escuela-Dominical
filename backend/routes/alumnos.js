...
router.get('/', verifyToken, async (req, res) => {
  try {
    ...
    console.log('Obteniendo alumnos de clase:', maestroClase);
    ...
    console.error('Error obteniendo alumnos:', error.message || error);
...
router.post('/', verifyToken, async (req, res) => {
  try {
    ...
    // Usar código si se proporciona, sino null
    const codigoFinal = codigo || null;
    ...
    console.error('Error creando alumno:', error.message || error);
    res.status(500).json({ error: error.message || 'Error interno del servidor' });
  }
});
...
router.put('/:id', verifyToken, async (req, res) => {
  try {
    ...
    // Si no se proporciona código, usar el existente del alumno
    let finalCodigo = codigo;
    if (!finalCodigo) {
      const existing = await Alumno.findById(id);
      if (!existing) return res.status(404).json({ error: 'Alumno no encontrado' });
      finalCodigo = existing.codigo || null;
    }
    ...
    console.error('Error actualizando alumno:', error.message || error);
    res.status(500).json({ error: error.message || 'Error interno del servidor' });
  }
});
...
router.delete('/:id', verifyToken, async (req, res) => {
  try {
    ...
    console.error('Error eliminando alumno:', error.message || error);
    res.status(500).json({ error: error.message || 'Error interno del servidor' });
  }
});
