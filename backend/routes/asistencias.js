...
router.post('/', verifyToken, async (req, res) => {
  try {
    ...
    if (!['presente', 'tardanza', 'ausente'].includes(estado)) {
...
router.post('/multiple', verifyToken, async (req, res) => {
  try {
    ...
    console.log(`📝 Guardando asistencias - Maestro: ${maestroClase}, Fecha: ${fecha}, Total enviados: ${asistencias.length}`);
    ...
    for (const asistencia of asistencias) {
      ...
      if (alumnoRows.length === 0) {
        console.warn(`⚠️ Alumno ${alumno_id} no encontrado en clase ${maestroClase}`);
        continue;
      }
    ...
    console.log(`📊 Total guardado: ${resultados.filter(r => r.success).length}/${resultados.length}`);
